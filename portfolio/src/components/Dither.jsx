...
function DitheredWaves({
  ...
  useEffect(() => {
    ...
    const newW = Math.floor(size.width * dpr);
    const newH = Math.floor(size.height * dpr);
    const currentRes = waveUniformsRef.current.resolution.value;
    if (currentRes.x !== newW || currentRes.y !== newH) {
      currentRes.set(newW, newH);
      // Actualiza resolución del RetroEffect si existe
      if (
        effectRef.current &&
        effectRef.current.uniforms &&
        effectRef.current.uniforms.resolution &&
        effectRef.current.uniforms.resolution.value
      ) {
        effectRef.current.uniforms.resolution.value.set(newW, newH);
      }
    }
  }, [size, gl]);
  ...
  return (
    <>
      ...
      <EffectComposer>
        <RetroEffect ref={effectRef} colorNum={colorNum} pixelSize={pixelSize} />
      </EffectComposer>
